...
class DoubleMaStrategy(CtaTemplate):
    ...
    def onBar(self, bar):
        ...
        if crossOver:
            ...
            print str(datetime.now()) + "\t【交易日】" ,bar.date
            print str(datetime.now()) + '\t【指标】',self.fastMa0,self.slowMa0,self.fastMa1,self.slowMa1
            # 如果金叉时手头没有持仓，则直接做多
            if self.pos == 0:
                self.buy(bar.close, self.fixedSize)
                
                print str(datetime.now()) + '\t【操作】','做多：',self.fixedSize
            # 如果有空头持仓，则先平空，再做多
            elif self.pos < 0:
                self.cover(bar.close, abs(self.pos))
                self.buy(bar.close, self.fixedSize)

                print str(datetime.now()) + '\t【操作】','平仓/做多：',abs(self.pos),self.fixedSize
            #如果金叉时手头还有持仓，则加多
            elif self.pos > 0:
                self.buy(bar.close, self.fixedSize)

                print str(datetime.now()) + '\t【操作】','加多：',self.fixedSize
        # 死叉和金叉相反
        elif crossBelow:
            print str(datetime.now()) +'\t'+'-'*30
            print str(datetime.now()) + "\t【交易日】" ,bar.date
            ...
            if self.pos == 0:
                ...
                print str(datetime.now()) + '\t【操作】','做空：',self.fixedSize
            elif self.pos > 0:
                self.sell(bar.close, abs(self.pos))
                self.short(bar.close, self.fixedSize)

                print str(datetime.now()) + '\t【操作】','平仓/做空：',abs(self.pos),self.fixedSize
             #如果死叉时手头还有持仓，则加空
            elif self.pos < 0:
                self.short(bar.close, self.fixedSize)
                print str(datetime.now()) + '\t【操作】','加空：',self.fixedSize
